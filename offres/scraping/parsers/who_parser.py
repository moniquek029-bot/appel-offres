# offres/scraping/parsers/who_parser.py
"""
Parser pour l'OMS (WHO) - Organisation Mondiale de la Santé
"""
import re
import logging
...
class WHOParser(BaseScraper):
    ...
    def parse(self, soup: BeautifulSoup) -> list[dict]:
        ...
        for link in soup.find_all('a', href=True):
            ...
            if not any(kw in href.lower() for kw in ['procurement', 'tender', 'rfq', 'rfp']):
                continue
    ...
    def run(self) -> list[dict]:
        logger.info(f"🕷️ WHO scraping: {self.source_url}")
        try:
