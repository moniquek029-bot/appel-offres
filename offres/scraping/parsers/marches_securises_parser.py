"""
Parser spécifique pour Marchés Sécurisés (France)
...
class MarchesSecurisesParser(BaseScraper):
    """Parser spécifique pour Marchés Sécurisés"""
    ...
    def parse(self, soup: BeautifulSoup) -> list[dict]:
        ...
        # Marchés Sécurisés utilise une structure avec des cartes ou listes
        # Pattern 1 : Articles ou cartes
        articles = soup.find_all(['article', 'div'], class_=re.compile(r'card|offer|tender|market|annonce'))
        
        if not articles:
            # Pattern 2 : Lignes de tableau
            articles = soup.find_all('tr')
        
        if not articles:
            # Pattern 3 : Liens avec mots-clés
            for link in soup.find_all('a', href=True):
                texte = link.get_text(strip=True)
                if len(texte) > 30 and any(mot in texte.lower() for mot in [
                    'marché', 'appel', 'offre', 'avis', 'consultation'
                ]):
        ...
        for article in articles[:30]:
            try:
                # Titre
                titre = None
                for sel in ['h2', 'h3', 'h4', 'a', '.title', 'strong']:
    ...
    def run(self) -> list[dict]:
        """Exécute le scraping"""
        logger.info(f"🕷️ MarchesSecurisesParser: {self.source_url}")
        ...
        try:
            ...
            logger.info(f"✅ MarchesSecurisesParser: {len(offres_valides)} offres")
    ...
    def _enrichir_offre(self, candidate: dict) -> dict | None:
        ...
        pays = detecter_pays_smart(texte_complet, url=url_source, pays_defaut='FR')
