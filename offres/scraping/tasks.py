# offres/scraping/tasks.py - Version CORRIGÉE (Sécurisée & PDF Intelligent)
...
# =============================================================================
# CONFIGURATION - 100% RÉEL, PDF OPTIONNEL
# =============================================================================
USE_MOCK_FALLBACK = False  # ← TOUJOURS FALSE pour production
REQUIRE_PDF = False        # ← À FALSE : Accepte l'offre même si le téléchargement du PDF échoue
...
def get_parser_for_source(source):
    ...
    try:
        ...
        logger.info(f" SmartParser utilisé pour {domain}")
        return SmartParser(source.url_racine)
...
def run_scheduled_scraping_task(self, source_id=None):
    """Tâche Celery pour le scraping (PDF optionnel)"""
    if source_id:
    ...
    for source in sources:
        try:
            ...
            if not scraper:
                logger.warning(f" Source ignorée: {source.nom}")
                total_skipped += 1
                continue
            ...
            for data in raw_offres:
                ...
                # Extraction du PDF si possible (optionnel)
                if not data.get('url_tdr') and data.get('url_source'):
                    try:
                        ...
                        logger.debug(f" PDF non extrait: {pdf_err}")
                
                # Sauvegarde avec PDF optionnel
...
def save_offre_real(data: dict, source: SourceScraping, require_pdf: bool = False) -> str:
    """
    Sauvegarde une offre RÉELLE (PDF OPTIONNEL)
    ...
    if existing:
        ...
        # On tente de télécharger le PDF si disponible, sans forcer un require_pdf strict
        if url_tdr and not existing.fichier_pdf:
            ...
            if pdf_content:
                ...
                updated = True
                logger.info(f"✅ PDF mis à jour localement: {titre[:40]}...")
    ...
    # SECURITÉ CRITIQUE : Si aucune date de clôture n'est trouvée, on met par défaut J+30 
    if not date_clot:
        date_clot = date_pub + timezone.timedelta(days=30)
        logger.warning(f"⚠️ Date clôture manquante pour '{titre[:30]}'. Fixée par défaut à J+30.")
    
    #  Téléchargement du fichier PDF (Optionnel mais automatique si l'URL existe)
    fichier_pdf = None
    if url_tdr:
        pdf_content = fetch_and_validate_pdf(url_tdr, titre)
        if pdf_content:
            ...
            # Si le PDF est explicitement obligatoire et qu'il échoue, on rejette l'offre
    ...
    try:
        ...
        logger.info(f"✅ CRÉÉE: {titre[:40]}... | PDF: {'✓' if fichier_pdf else '🔗' if url_tdr else '✗'}")
        return 'created'
    except Exception as e:
...
def daily_archive_task():
    ...
    logger.info(" Archivage automatique...")
    try:
